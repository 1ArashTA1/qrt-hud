...
local DEFAULT_IMPOUND_TIME = 120 
...
QBCore.Commands.Add('depot', Lang:t('commands.depot'), {
    { name = 'price',  help = Lang:t('info.impound_price') },
    { name = 'time',   help = 'Lock time (minutes) - default: 120' },
    { name = 'reason', help = 'Impound reason (optional)' },
    ...
    if Player.PlayerData.job.type == 'leo' and Player.PlayerData.job.onduty then
        ...
        TriggerClientEvent('police:client:ImpoundVehicle', src, false, price, time, reason)
    else
        TriggerClientEvent('QBCore:Notify', src, Lang:t('error.on_duty_police_only'), 'error')
    end
end)

QBCore.Commands.Add('impound', Lang:t('commands.impound'), {
    { name = 'time',   help = 'Lock time (minutes) - default: 120' },
    { name = 'reason', help = 'Impound reason (optional)' },
}, false, function(source, args)
    local src = source
    ...
    if Player.PlayerData.job.type == 'leo' and Player.PlayerData.job.onduty then
        ...
        TriggerClientEvent('police:client:ImpoundVehicle', src, true, nil, time, reason)
    else
        TriggerClientEvent('QBCore:Notify', src, Lang:t('error.on_duty_police_only'), 'error')
    end
end)
            ...
            if biller.PlayerData.citizenid ~= billed.PlayerData.citizenid then
                if amount and amount > 0 then
                    if billed.Functions.RemoveMoney('bank', amount, 'paid-fine') then
                        ...
                        exports['qrt-banking']:AddMoney(biller.PlayerData.job.name, amount, 'Fine')
...
RegisterNetEvent('police:server:TakeOutImpound', function(plate, garage)
    ...
    -- ✅ آزادسازی کامل: وضعیت + تایمر قفل + دلیل اینپاند ریست می‌شن
    MySQL.update('UPDATE player_vehicles SET state = ?, impoundtime = ?, impoundreason = ? WHERE plate = ?',
        { 0, 0, '', plate })
...
RegisterNetEvent('police:server:BillPlayer', function(playerId, price)
    ...
    exports['qrt-banking']:AddMoney('police', price, 'Fine paid')
...
RegisterNetEvent('police:server:Impound', function(plate, fullImpound, price, body, engine, fuel, lockMinutes, reason)
    local src = source
    price = price or 0

    -- ✅ تایم: اگه پلیس نزده → پیش‌فرض 2 ساعت
    local minutes = (tonumber(lockMinutes) and tonumber(lockMinutes) > 0) and tonumber(lockMinutes) or DEFAULT_IMPOUND_TIME
    local releaseAt = os.time() + (minutes * 60)

    -- ✅ دلیل: اگه پلیس ننوشته → اسم افسر ثبت می‌شه
    local officer = 'Unknown Officer'
    local Player = QBCore.Functions.GetPlayer(src)
    if Player then
        officer = Player.PlayerData.charinfo.firstname .. ' ' .. Player.PlayerData.charinfo.lastname
    end
    local impoundReason = (reason and tostring(reason) ~= '') and tostring(reason) or ('Impounded by Officer ' .. officer)

    if IsVehicleOwned(plate) then
        -- ✅ هر دو دستور حالا state=2 (اینپاند) + تایمر + دلیل + قیمت
        MySQL.query(
            'UPDATE player_vehicles SET state = 2, depotprice = ?, body = ?, engine = ?, fuel = ?, impoundtime = ?, impoundreason = ? WHERE plate = ?',
            { price, body, engine, fuel, releaseAt, impoundReason, plate },
            function()
                TriggerClientEvent('QBCore:Notify', src, ('Vehicle %s impounded for %d minutes'):format(plate, minutes), 'success')
            end
        )
    end
end)
